/**
 * Harvest Prediction Runner — single source of truth
 *
 * Shared by:
 *  - scripts/harvest-predict.ts (manual run: `npx tsx scripts/harvest-predict.ts`)
 *  - src/app/api/cron/harvest-predict/route.ts (Vercel cron)
 *
...
export async function runHarvestPrediction(): Promise<PredictResult> {
  ...
  if (!farm) {
    throw new Error('[Predict] Brak farmy z danymi zbiorowymi')
  }
  ...
  return { farm: farm.name, todayStr, updated, created }
}
